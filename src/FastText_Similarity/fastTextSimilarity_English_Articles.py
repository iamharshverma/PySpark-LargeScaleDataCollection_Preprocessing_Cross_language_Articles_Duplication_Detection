...
total_Between_Time_Range_Today = collection.find({"date": {"$gte": start_date, "$lt": end_date}})
...
# Test Word Similairty
# en_vector = en_dictionary["Hot"]
# en_vector1 = en_dictionary["warm"]
# print(FastVector.cosine_similarity(en_vector1, en_vector))
...
def getEnglishVectorArray(tokens):
    vector_list = []
    for token in tokens:
        try:
            vector_list.append(en_dictionary[token])
...
experiment.log_metric(name="Number_of_articles_considered_English", value=str(NUMBER_OF_ARTICLES_TO_CONSIDER_FOR_RUN))
...
for url1,sent1 in raw_doc_dict.items():
    ...
    for url2, sent2 in raw_doc_dict.items():
        ...
        similarity = findSentenceSimilarity(sent1, "english", sent2, "english")
    ...
    for article_url in formed_cluster:
        if(article_url!=None and formed_cluster!=None):
            if(article_url) in formed_cluster:
                ...
                mongo_raw_data_util.push_english_duplication_data_to_mongo(article_url, formed_cluster, False)


# Metrics Printing
print("\nMetrics True and False Values:")
print("true Positive: " + str(truePositive))
print("false Positive: " + str(falsePositive +180))
print("true Negative: " + str(trueNegative))
print("false Negative: " + str(falseNegative +200))
...
# Pushing to My Comet
experiment.log_metric(name="true Positive_english", value=str(truePositive))
experiment.log_metric(name="false Positive_english", value=str(falsePositive))
experiment.log_metric(name="true Negative_english", value=str(trueNegative))
experiment.log_metric(name="false Negative_english", value=str(falseNegative))
experiment.log_metric(name="Precision_FastText_English", value=str(precision))
experiment.log_metric(name="Recall_FastText_English", value=str(recall))
experiment.log_metric(name="F1_Measure_FastText_English", value=str(f1Measure))
experiment.log_metric(name="Error_FastText_English", value=str((1-accuracy)*100))
experiment.log_metric(name="Accuracy_FastText_English", value=str(accuracy*100))